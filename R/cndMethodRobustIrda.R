...
#' \describe{
#'  \item{\code{transformation}}{to normalize the data, \code{X} is first transformed as clr (function \code{cndClr}). However, because the this transformation makes the covariance matrix is not invertible, a condition required to compute the Mahalanobis distance, a nutrient is dropped after the transformation. The \code{fill} component is used by default, but any nutrient can be chosen by precising \code{dropNutrient} as an entry of the \code{cndMethodRobustIrda} function. The approach was preconised by Parent et al. (2009).}
...
#'  \item{\code{norm}}{a robust norm is then obtained through the minimum covariance determinant estimation (function \code{cndMcd}). The returned object is of class \code{McdNorm}, which inherits from the class \code{CndNorm}. The slot \code{subset} of the \code{McdNorm} object (representing further subseting by the MCD) can be used to associate a yield to the norm. However, this yield might not be representative for another dataset. The method, brought by IRDA, replaces the iterative method of outlier detection used by Parent el al. (2009) to eliminate unbalanced cases that generate high yield (e.g. plants that grew in soils rich enough to reach maximum yield might be less influenced by balances).}
