...
#' @description S4 class that extend the class \code{CndNorm} to also include the logarithm of the covariance matrix determinant \code{logDet} associated to the robust method and subsets involved in the calculation of the location and scatter.
...
#' @slot subsetRaw a \code{CndData} object, representing the subset of observation that minimized the determinant.
#' @slot subsetReweighted a \code{CndData} object, representing the subset of observation resulting from the reweighting.
...
#'
setClass("McdNorm", slots = c(logDet = "numeric", subsetRaw = "CndData", subsetReweighted = "CndData"), contains = "CndNorm")
...
#' @rdname S4Class-McdNorm
#' @export
McdNorm = function(location = numeric(0), scatter = matrix(numeric(0), ncol = 0, nrow = 0), logDet = numeric(0), subsetRaw = CndData(), subsetReweighted = CndData()){
  #Generate the object of class RobustMSD. Allows empty object, but does not validate dimensions.
  object <- new("McdNorm", location = location, scatter = scatter, logDet = logDet, subsetRaw = subsetRaw, subsetReweighted = subsetReweighted)
  return(object)
}
#--------------------------------------------------------------------
