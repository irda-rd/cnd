...
X_na <- X1; X_na[1,1] <- NA
yield_na <- yield1; yield_na[1] <- NA

test_that("unadequate objects cannot be constructed (number of lines)", {
...
test_that("unadequate objects cannot be constructed (number of columns)", {
  expect_error(CndData(yield = yield_doubled, X = X1, label = label))
})

test_that("unadequate objects cannot be constructed (numeric classes)", {
  expect_error(CndData(yield = yield_character, X = X1))
  expect_error(CndData(yield = yield1, X = X_character))
})

test_that("unadequate objects cannot be constructed (NA in yield or X)", {
